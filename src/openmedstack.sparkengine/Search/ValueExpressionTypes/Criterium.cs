...
namespace OpenMedStack.SparkEngine.Search.ValueExpressionTypes
{
    ...
    public class Criterium : Expression, ICloneable
    {
        ...
        private static Criterium FromPathTuples(Tuple<string, string?>[] path, string value, string? resourceType = null)
        {
            var first = path[0];
            ...
            {
                type = Operator.CHAIN;
                operand = FromPathTuples(path[1..], value, resourceType);
            }
