...
namespace OpenMedStack.SparkEngine.Core
{
    ...
    public static class FhirMediaType
    {
        ...
        public static string Interpret(string? format)
        {
            if (format == null)
            {
                return DefaultJsonMimeType;
            }

            if (XmlMimeTypes.Contains(format))
            {
                return DefaultXmlMimeType;
            }

            if (JsonMimeTypes.Contains(format))
            {
                return DefaultJsonMimeType;
            }
        ...
        public static ResourceFormat GetResourceFormat(string format)
        {
            ...
            if (strict == DefaultXmlMimeType)
            {
                return ResourceFormat.Xml;
            }
            else if (strict == DefaultJsonMimeType)
            {
