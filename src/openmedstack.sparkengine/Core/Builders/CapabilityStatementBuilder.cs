...
namespace OpenMedStack.SparkEngine.Core.Builders
{
    ...
    public class CapabilityStatementBuilder
    {
        ...
        public CapabilityStatement Build()
        {
            ...
            if (_url != null)
            {
                capabilityStatement.UrlElement = _url;
            }

            if (_version != null)
            {
                capabilityStatement.VersionElement = _version;
            }

            if (_name != null)
            {
                capabilityStatement.NameElement = _name;
            }

            if (_title != null)
            {
                capabilityStatement.TitleElement = _title;
            }

            if (_status != null)
            {
                capabilityStatement.StatusElement = _status;
            }

            if (_experimental != null)
            {
                capabilityStatement.ExperimentalElement = _experimental;
            }

            if (_date != null)
            {
                capabilityStatement.DateElement = _date;
            }

            if (_publisher == null)
            {
                capabilityStatement.PublisherElement = _publisher;
            }

            if (_contact is { Count: > 0 })
            {
                capabilityStatement.Contact = _contact;
            }

            if (_description != null)
            {
                capabilityStatement.Description = _description;
            }

            if (_useContext is { Count: > 0 })
            {
                capabilityStatement.UseContext = _useContext;
            }

            if (_jurisdiction is { Count: > 0 })
            {
                capabilityStatement.Jurisdiction = _jurisdiction;
            }

            if (_purpose != null)
            {
                capabilityStatement.Purpose = _purpose;
            }

            if (_copyright != null)
            {
                capabilityStatement.Copyright = _copyright;
            }

            if (_kind != null)
            {
                capabilityStatement.KindElement = _kind;
            }

            if (_instantiates is { Count: > 0 })
            {
                capabilityStatement.InstantiatesElement = _instantiates;
            }

            if (_imports is { Count: > 0 })
            {
                capabilityStatement.ImportsElement = _imports;
            }

            if (_software != null)
            {
                capabilityStatement.Software = _software;
            }

            if (_implementation != null)
            {
                capabilityStatement.Implementation = _implementation;
            }

            if (_fhirVersion != null)
            {
                capabilityStatement.FhirVersionElement = _fhirVersion;
            }

            if (_format is { Count: > 0 })
            {
                capabilityStatement.FormatElement = _format;
            }

            if (_patchFormat is { Count: > 0 })
            {
                capabilityStatement.PatchFormatElement = _patchFormat;
            }

            if (_implementationGuide is { Count: > 0 })
            {
                capabilityStatement.ImplementationGuideElement = _implementationGuide;
            }

            if (_rest is { Count: > 0 })
            {
                capabilityStatement.Rest = _rest;
            }

            if (_messaging is { Count: > 0 })
            {
                capabilityStatement.Messaging = _messaging;
            }

            if (_document != null)
            {
                capabilityStatement.Document = _document;
            }
