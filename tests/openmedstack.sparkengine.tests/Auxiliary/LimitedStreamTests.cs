...
namespace OpenMedStack.SparkEngine.Tests.Auxiliary;

using System;
using System.IO;
using SparkEngine.Auxiliary;
using Xunit;
...
public class LimitedStreamTests
{
    ...
    public async Task TestCopyToAsyncAboveLimit()
    {
        ...
        var sourceStream = new MemoryStream([1, 2, 3, 4, 5]);

        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() => sourceStream.CopyToAsync(sut));
    }
}
